...
function createFilterRoutes(path, _get=() => Book.find({})) {
    ...
    router.get(`/${path}/props/:fields`, (req, res) => {
        ...
        req.params.fields.split(',').forEach((field) => {
            ...
            if (!Book.pathExists(field) && !res.headersSent) return res.status(400).send({message: `Invalid property: ${field}`});
...
createFilterRoutes('by-:prop/:val', (req, res) => {
    ...
    if (prop === 'id') prop = '_id';
    if (!Book.pathExists(prop)) return res.status(400).send({message: `Invalid property: ${prop}`});
    ...
    if (Book.pathType(prop) === "String") {
        ...
        if (val.startsWith(">=")) {
            ...
            if (isNaN(val)) return res.status(400).send({message: `Invalid Number: ${val}`});
            ...
            if (isNaN(min)) return res.status(400).send({message: `Invalid Min Number: ${min}`});
            if (isNaN(max)) return res.status(400).send({message: `Invalid Max Number: ${max}`});
